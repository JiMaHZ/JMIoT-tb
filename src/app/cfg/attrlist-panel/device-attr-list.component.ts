...
export class DeviceAttrListComponent implements OnInit {
  ...
  getDeviceAttrList(deviceid:string): void {
     this.http.get(`http://${environment.serverUrl}/api/plugins/telemetry/DEVICE/`+deviceid+`/values/attributes/SERVER_SCOPE`,this.httpOption)
  ...
  deleteDevice(): void{
    if(this.chkkey.length>0){
      if(confirm("您确定要删除所选的对象吗？")){
        this.http.delete(`http://${environment.serverUrl}/api/plugins/telemetry/`+this.cur_device_id+`/SERVER_SCOPE?keys=`+this.chkkey,this.httpOption).toPromise()
        .then(response => {
  ...
  doEditDeviceAttr(deviceAttr:DeviceAttr):Promise<any>{
    ...
    return this.http.post(`http://${environment.serverUrl}/api/plugins/telemetry/DEVICE/`+this.cur_device_id+`/SERVER_SCOPE`, JSON.parse(JSON.stringify(str)),this.httpOption).toPromise()
      .then(response => {
        if (response.status == 200) {
